...
export default {
  init(container, config = {}) {
    ...
    const drawBackdrop = (w, h, elapsed) => {
      ...
      const ember = ctx.createRadialGradient(w * 0.5, h * 0.55, 0, w * 0.5, h * 0.55, Math.min(w, h) * 0.35);
      ember.addColorStop(0, `rgba(255,80,18,${0.16 + intensity * 0.18})`);
    ...
    const drawAmbient = (w, h, elapsed) => {
      ambient.forEach((a, i) => {
        ...
        const glow = ctx.createRadialGradient(x, y, 0, x, y, a.r);
        glow.addColorStop(0, `rgba(0,0,0,${0.85 * a.life})`);
        glow.addColorStop(0.65, `rgba(28,0,0,${0.56 * a.life})`);
        glow.addColorStop(1, `rgba(255,40,0,${0.2 + intensity * 0.2 * a.life})`);
        ctx.fillStyle = glow;
        ctx.beginPath();
        ctx.arc(x, y, a.r * (0.8 + 0.2 * Math.sin(elapsed * 1.7 + a.phase)), 0, Math.PI * 2);
        ctx.fill();
      });
    };
    ...
    const drawFlameParticle = (f) => {
      ...
      grad.addColorStop(0.62, `rgba(26,0,0,${0.86 * f.life})`);
      grad.addColorStop(1, `rgba(255,0,0,${0.72 * f.life})`);
      ctx.fillStyle = grad;
      ctx.beginPath();
      ...
      ctx.strokeStyle = `rgba(255,48,0,${0.45 * f.life})`;
      ctx.lineWidth = 1.3;
      ctx.beginPath();
      ctx.arc(f.x, f.y, f.size * 0.92, 0, Math.PI * 2);
      ctx.stroke();
      ctx.restore();
    };
